...
TEST_IMAGE="${1:?uso: test-deploy-rollback.sh IMAGEN}"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
TEST_ROOT="$(mktemp -d)"
PREFIX="registry-deploy-rollback-$$-$RANDOM"
...
docker run --rm -i \
  --entrypoint sh \
  -v "$REMOTE_VOLUME:/remote" \
  "$TEST_IMAGE" -eu -c '
mkdir -p /remote/bin
cat > /remote/bin/remote-command
for command_name in docker curl sleep; do
...
mkdir -p /remote/data /remote/containers /remote/bin
...
ssh() {
  ...
  case "$remote" in
    ...
    *"docker inspect 'Local-Registry' --format '{{.State.Running}}"*)
      printf '%s\n' 'true|local-registry:v1.2.12|unless-stopped|5000|5001'
      ;;
    *'envs="$(docker inspect'*)
      printf '%s\n' secure
      ;;
    *"docker pull -q 'local-registry:v1.2.12'"*)
      return 0
      ;;
    ...
    *"rm -f '/remote/secrets/.admin-token."*)
      ...
      ;;
    *)
      # Lecturas de una plantilla que no afectan a esta regresión: sin valor.
      return 1
      ;;
  esac
}
...
if [[ "$status" -eq 0 ]]; then
  echo "El despliegue simulado no propagó el fallo de salud" >&2
  exit 1
fi
grep -Fq 'contenedor, secretos y plantilla anteriores restaurados' <<< "$output"
if grep -Fq "$OLD_TOKEN" <<< "$output" || grep -Fq "$NEW_TOKEN" <<< "$output" \
    || grep -Fq "$OLD_TOKEN" "$SSH_LOG" || grep -Fq "$NEW_TOKEN" "$SSH_LOG"; then
  echo "Un secreto apareció en la salida o en argv SSH" >&2
  exit 1
fi
docker_log="$(docker run --rm --entrypoint cat \
  -v "$REMOTE_VOLUME:/remote" "$TEST_IMAGE" /remote/docker.log)"
if grep -Fq "$OLD_TOKEN" <<< "$docker_log" || grep -Fq "$NEW_TOKEN" <<< "$docker_log"; then
  echo "Un secreto apareció en los argumentos de docker run" >&2
  exit 1
fi

expected_hash="$(printf '%s\n' "$OLD_TOKEN" | sha256sum | cut -d' ' -f1)"
actual_hash="$(docker run --rm --entrypoint sha256sum \
  -v "$REMOTE_VOLUME:/remote" "$TEST_IMAGE" /remote/secrets/admin-token \
  | cut -d' ' -f1)"
[[ "$actual_hash" == "$expected_hash" ]]

docker run --rm --entrypoint sh -v "$REMOTE_VOLUME:/remote" "$TEST_IMAGE" -eu -c '
test -f /remote/containers/Local-Registry
test -z "$(find /remote/containers -maxdepth 1 -name "Local-Registry-rollback-*" -print -quit)"
test -z "$(find /remote/secrets -maxdepth 1 -name ".rollback-*" -print -quit)"
test -z "$(find /remote/secrets -maxdepth 1 -name ".admin-token.*.new" -print -quit)"
...
test -z "$(find "$(dirname "$template")" -maxdepth 1 \
  \( -name "my-Local-Registry.xml.*.new" -o -name "my-Local-Registry.xml.*.rollback" \) \
  -print -quit)"
grep -q "^rename Local-Registry Local-Registry-rollback-" /remote/docker.log
grep -q "^stop Local-Registry-rollback-" /remote/docker.log
grep -q "^run -d --name Local-Registry " /remote/docker.log
grep -q "^rm -f Local-Registry$" /remote/docker.log
grep -q "^rename Local-Registry-rollback-.* Local-Registry$" /remote/docker.log
grep -q "^start Local-Registry$" /remote/docker.log
'

echo "Rollback seguro: fallo de salud, secreto previo y contenedor anterior restaurados sin exponer valores."
